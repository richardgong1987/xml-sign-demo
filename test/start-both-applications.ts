...
import {createDemoAccessTokenSecret} from "../src/service-provider/services/demo-access-token-secret";
import {
...
export async function startBothApplications(ports: {
    ...
    const serviceProvider = await listen(
        await createWebApplication({
            module: ServiceProviderModule.register({
                config: serviceProviderConfig,
                accessTokenSecret: createDemoAccessTokenSecret(),
            }),

...
describe("single sign-on", () => {
    ...
    it("hands the browser a JWT and points it at the requested page", async () => {
        const handoff = await runSingleSignOn({uid: "hanjin"});
    ...
    it("returns the user to the page they wanted before signing in", async () => {
        const handoff = await runSingleSignOn({uid: "sakura", returnTo: "/orders/42"});

        expect(handoff.returnTo).toBe("/orders/42");
    });

    it("answers /api/me with the asserted user when the token is presented", async () => {
        const {accessToken} = await runSingleSignOn({uid: "hanjin"});
        ...
        expect(await response.json()).toEqual({
            fields: [
                {label: "NameID", value: "hanjin"},
                {label: "uid", value: "hanjin"},
                {label: "email", value: "hanjin@example.test"},
                {label: "role", value: "trader"},
...
describe("access token", () => {
    ...
    it("rejects a token whose payload was edited after signing", async () => {
        ...
        const response = await get(`${serviceProviderBaseUrl}/api/me`, promoteToAdministrator(accessToken));
...
describe("rejected assertions", () => {
    it("refuses to issue a token after a man-in-the-middle edits role", async () => {
        const response = await postAssertion(await issueSamlResponse({uid: "hanjin", shouldTamper: true}));
    ...
    it("rejects a replayed SAMLResponse", async () => {
        const autoPostForm = await issueSamlResponse({uid: "hanjin"});

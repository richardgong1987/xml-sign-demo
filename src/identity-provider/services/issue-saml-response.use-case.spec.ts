...
/*
 * A fake AssertionSigner: it records the XML it is handed and performs no
 * cryptography, so the use case can be tested without any key material.
 */
...
describe("IssueSamlResponseUseCase", () => {
    ...
    it("stamps the assertion with the time from the injected clock", () => {
        ...
        expect(assertionSigner.signedDocuments[0]).toMatch(
            /IssueInstant="2026-07-28T09:00:00\.000Z"/,
        );
    });

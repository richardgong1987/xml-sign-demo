...
describe("AuthnRequestParser", () => {
    ...
    it("reads the request ID and the issuing SP from a redirect-binding request", () => {
        expect(parser.parseRedirectBinding(encodeRedirectBinding(authnRequestXml()))).toEqual({
  ...
  <saml:Issuer>
    ...
    it("returns a frozen model so no later stage can rewrite the issuer", () => {
        const authnRequest = parser.parseRedirectBinding(encodeRedirectBinding(authnRequestXml())) as {

import {IDENTITY_PROVIDER_BASE_URL, SERVICE_PROVIDER_BASE_URL} from "./ports";
import {
    TokenHandoff,
    get,
    postForm,
...
/*
 * Drives the whole SP-initiated SSO flow over real HTTP: a NestJS identity provider and
 * a Next.js service provider, each started from its own workspace build.
 *
 * Both are real servers — `node dist/main.js` and `next start` — so this exercises the
 * actual route handlers, not an in-process stand-in.
 */
...
describe("trust establishment", () => {
    it("publishes the signing certificate and an HTTP-Redirect SSO endpoint in the IdP metadata", async () => {
        const response = await get(`${IDENTITY_PROVIDER_BASE_URL}/idp/metadata`);
    ...
    it("publishes the ACS URL and demands signed assertions in the SP metadata", async () => {
        const response = await get(`${SERVICE_PROVIDER_BASE_URL}/api/saml/metadata`);
        const metadata = await response.text();

        expect(response.status).toBe(200);
        expect(metadata).toMatch(/WantAssertionsSigned="true"/);
        expect(metadata).toContain(`${SERVICE_PROVIDER_BASE_URL}/api/saml/acs`);
    });
});
...
describe("single sign-on", () => {
    it("redirects to the IdP carrying SAMLRequest and RelayState", async () => {
        ...
        expect(authorizeUrl.origin + authorizeUrl.pathname).toBe(
            `${IDENTITY_PROVIDER_BASE_URL}/idp/sso`,
        );
    ...
    it("shows the AuthnRequest context the IdP parsed", async () => {
        ...
        expect(loginPage.html).toContain(`${SERVICE_PROVIDER_BASE_URL}/api/saml/metadata`);
    ...
    it("hands the browser a JWT and points it at the requested page", async () => {
        const handoff = await runSingleSignOn({});
    ...
    it("returns the user to the page they wanted before signing in", async () => {
        const handoff = await runSingleSignOn({returnTo: "/orders/42"});

        expect(handoff.returnTo).toBe("/orders/42");
    });

    it("answers /api/me with the asserted user when the token is presented", async () => {
        const {accessToken} = await runSingleSignOn({});

        const response = await get(`${SERVICE_PROVIDER_BASE_URL}/api/me`, accessToken);
    ...
    it("serves /profile as a shell that verifies the token client-side", async () => {
        ...
        const response = await get(`${SERVICE_PROVIDER_BASE_URL}/profile`);
...
describe("access token", () => {
    it("rejects a request that carries no token", async () => {
        expect((await get(`${SERVICE_PROVIDER_BASE_URL}/api/me`)).status).toBe(401);
    });

    it.each(["not-a-jwt", "a.b.c"])("rejects a malformed token (%p)", async (token) => {
        expect((await get(`${SERVICE_PROVIDER_BASE_URL}/api/me`, token)).status).toBe(401);
    });
    ...
    it("rejects a token whose payload was edited after signing", async () => {
        ...
        const response = await get(
            `${SERVICE_PROVIDER_BASE_URL}/api/me`,
    ...
    it("still accepts a token after the browser signed out, because nothing revokes it", async () => {
        ...
        expect((await get(`${SERVICE_PROVIDER_BASE_URL}/api/me`, accessToken)).status).toBe(200);
    });
});

describe("rejected assertions", () => {
    it("refuses to issue a token after a man-in-the-middle edits role", async () => {
        const response = await postAssertion(await issueSamlResponse({shouldTamper: true}));
    ...
    it("rejects a replayed SAMLResponse", async () => {
        const autoPostForm = await issueSamlResponse({});
    ...
    it("rejects a malformed AuthnRequest at the IdP", async () => {
        const response = await get(`${IDENTITY_PROVIDER_BASE_URL}/idp/sso?SAMLRequest=%21%21%21bad`);
...
async function startLogin(returnTo: string): Promise<string> {
    const response = await get(
        `${SERVICE_PROVIDER_BASE_URL}/login?returnTo=${encodeURIComponent(returnTo)}`,
    );
...
async function issueSamlResponse({
    ...
    // Whoever the IdP's demo directory holds — the spec should not care about the fixture.
    const uid = loginPage.html.match(/name="uid" value="([^"]*)"/)![1];

    const html = await (
        await postForm(`${IDENTITY_PROVIDER_BASE_URL}/idp/login`, {
            ...loginPage.fields,
            uid,
